...
import { Plus, Edit2, Trash2, Clock, Tag, Megaphone, Film, QrCode, Search, AlertTriangle, X, Upload, Layers } from "lucide-react";
...
export default function MoviesPage() {
  ...
  // Helper to get image URL from thumbnails folder
  ...
  const remove = async (id: string) => {
    ...
    try {
      const list = getMovies().filter((m) => m.id !== id);
      await saveMoviesToApi(list);
      setMovies(list);
      await refresh();
  ...
  const addAdsFromFiles = (type: AdType, files: File[]) => {
    ...
    setDraft((d) => {
      ...
      const newAds: Ad[] = files.map((f, i) => {
        ...
        if (type === "midroll") ad.timestamp = (existingMid + i + 1) * 600;
        return ad;
      });
  ...
  return (
    <DashboardLayout>
      {/* Intro Section */}
      <div className="mb-8 rounded-2xl gradient-card border border-border overflow-hidden">
        <div className="p-6 md:p-8">
          <div className="flex items-start gap-4">
            <div className="p-3 rounded-xl bg-primary/10 shrink-0">
              <Film className="w-8 h-8 text-primary-glow" />
            </div>
            <div className="flex-1 min-w-0">
              <h1 className="text-2xl md:text-3xl font-bold flex items-center gap-2">
                Movie Catalog
                <span className="text-xs font-normal px-2 py-0.5 rounded-full bg-primary/20 text-primary-glow">
                  {movies.length} {movies.length === 1 ? 'movie' : 'movies'}
                </span>
              </h1>
              <p className="text-muted-foreground mt-1">
                Manage your streaming library — add, edit, and organize your movie collection.
              </p>
              <div className="flex flex-wrap gap-4 mt-3 text-xs text-muted-foreground">
                <div className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-full bg-success"></div>
                  <span>{movies.filter(m => m.published).length} Published</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-full bg-warning"></div>
                  <span>{movies.filter(m => !m.published).length} Drafts</span>
                </div>
                <div className="flex items-center gap-1.5">
            ...
            <div className="hidden sm:flex items-center gap-2 shrink-0">
              <button
                ...
                className="px-4 py-2 rounded-xl bg-secondary hover:bg-secondary/70 text-sm font-medium flex items-center gap-2 transition-colors"
              >
                <Upload className="w-4 h-4" /> Import
              </button>
              <button
                onClick={openNew}
                className="px-4 py-2 rounded-xl gradient-brand text-primary-foreground text-sm font-semibold flex items-center gap-2 hover:shadow-glow transition-all"
              >
      ...
      <header className="flex items-center justify-between mb-8 sm:hidden">
        <div>
      ...
      {movies.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredMovies.map((m) => {
            ...
            const posterUrl = getImageUrl(m.poster);
            return (
                    ...
                    <img 
                      ...
                      onError={(e) => {
                        ...
                        if (parent) {
                          ...
                          placeholder.innerHTML = `<svg class="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 4v16M17 4v16M3 8h18M3 16h18"/></svg>`;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Field label="Poster">
              <DropUpload
                ...
                kind="image"
                aspect="poster"
                label="Poster image (2:3 ratio recommended)"
              />
            </Field>
            <Field label="Backdrop">
              <DropUpload
                ...
                kind="image"
                aspect="wide"
                label="Backdrop image (16:9 ratio recommended)"
              />
            </Field>
          </div>
